...
export default function AuthPage() {
  ...
  const handleGoogleContinue = () => {
    ...
    if (!publicAppOrigin && !isGoogleOAuthSupportedOrigin(window.location)) {
      setError("Google sign-in needs localhost, an HTTPS domain, or NEXT_PUBLIC_APP_ORIGIN. Use email/password on this LAN URL.");
      return;
    }
      ...
      <div className="relative hidden overflow-hidden bg-hero-gradient lg:flex lg:flex-col lg:justify-between lg:p-12 lg:text-primary-foreground">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-[10px] uppercase tracking-[0.14em] text-primary-foreground/60">{BRAND_SUBTITLE}</div>
          </div>
        </div>

        <div className="relative">
          <Image src={trophyHero} alt="World Cup trophy" priority className="absolute -right-28 -top-40 w-[640px] opacity-90 mix-blend-screen" />
          <div className="relative max-w-md">
            ...
            <div className="text-sm font-medium uppercase tracking-[0.18em] text-gold">{BRAND_SUBTITLE} presents</div>
            <h1 className="mt-3 text-5xl font-semibold leading-[1.05] tracking-tight">{BRAND_TITLE}</h1>
            <p className="mt-5 text-lg text-gold">Predict every match. Climb the leaderboard.</p>
            <p className="mt-4 text-base text-primary-foreground/70">Create private leagues, challenge colleagues, earn points, and compete for the top spot throughout the tournament.</p>
          </div>
        </div>

        <div className="grid max-w-md grid-cols-3 gap-3">
          {[
            { icon: Target, label: "Exact score", value: "5 pts" },
            { icon: Trophy, label: "Correct winner", value: "1 pt" },
            { icon: Flame, label: "Streak bonus", value: "Daily" },
          ].map((s) => (
...
function Field({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <label className="block">
      ...
      <div className="relative">
        <input {...props} type={isPasswordField && showPassword ? "text" : props.type} className="w-full rounded-xl border border-input bg-card py-2.5 pl-3.5 pr-11 text-sm outline-none ring-ring/0 transition focus:border-ring focus:ring-2 focus:ring-ring/30" />
        {isPasswordField && (
          <button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Hide password" : "Show password"} className="absolute inset-y-0 right-0 flex items-center px-3 text-muted-foreground transition hover:text-foreground">
...
function GoogleIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4">
      <path fill="#EA4335" d="M12 10.2v3.9h5.4c-.2 1.3-1.5 3.9-5.4 3.9-3.2 0-5.9-2.7-5.9-6s2.7-6 5.9-6c1.8 0 3 .8 3.7 1.4l2.5-2.4C16.6 3.5 14.5 2.6 12 2.6 6.9 2.6 2.8 6.7 2.8 11.8S6.9 21 12 21c6.9 0 8.6-4.8 8.6-7.3 0-.5-.1-.9-.1-1.2z" />
      <path fill="#4285F4" d="M21.9 12.5c0-.5-.1-.9-.1-1.2H12v3.9h5.4c-.3 1.4-1.1 2.5-2.3 3.3l3.5 2.7c2-1.8 3.3-4.5 3.3-8.7z" />
      <path fill="#FBBC05" d="M6.1 14.3c-.2-.6-.3-1.3-.3-2s.1-1.4.3-2L3.4 8.2C2.7 9.6 2.3 11.1 2.3 12.3s.4 2.8 1.1 4.1z" />
      <path fill="#34A853" d="M12 21c2.5 0 4.7-.8 6.2-2.3l-3.5-2.7c-.9.6-2.1 1-3.5 1-2.7 0-5-1.8-5.8-4.2l-2.8 2.2C4.1 18.1 7.8 21 12 21z" />
      <path fill="#4285F4" d="M6.2 7.8C7 5.4 9.3 3.6 12 3.6c1.5 0 2.8.5 3.8 1.5l2.8-2.8C16.7 1 14.5.1 12 .1 7.8.1 4.1 3 2.6 6.9z" />
    </svg>
  );
}
