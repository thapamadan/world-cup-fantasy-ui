...
export function MatchesPageClient({ initialMatches, initialError }: { initialMatches: Match[]; initialError?: string }) {
  ...
  useEffect(() => {
    ...
    const scheduleRefresh = (nextMatches: Match[]) => {
      ...
      const refreshInterval = nextMatches.some((match) => match.status === "live") ? LIVE_REFRESH_INTERVAL_MS : DEFAULT_REFRESH_INTERVAL_MS;
